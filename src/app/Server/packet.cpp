...
serverPacket::serverPacket() noexcept
    : m_header{MSG_VERSION, MSG_DEFAULT, 0},
    m_message{}
{
}

serverPacket::serverPacket(const unsigned int version,
...
unsigned int serverPacket::get_version() const noexcept { return m_header.m_version; }
unsigned int serverPacket::get_message_type() const noexcept { return m_header.m_type; }
unsigned int serverPacket::get_length() const noexcept { return m_header.m_length; }
unsigned int serverPacket::get_header_size() const noexcept { return sizeof(m_header); }
unsigned int serverPacket::get_message_size() const noexcept { return m_message.length(); }
unsigned int serverPacket::get_packet_size() const noexcept { return get_header_size() + get_message_size(); }
std::string serverPacket::get_message() const noexcept { return m_message; }

void serverPacket::set_version(const unsigned int version) noexcept { m_header.m_version = version; }
void serverPacket::set_type(const unsigned int type) noexcept { m_header.m_type = type; }
void serverPacket::set_length(const unsigned int length) noexcept { m_header.m_length = length; }
void serverPacket::set_message(const std::string message) noexcept { m_message = message; }

void serverPacket::set(const unsigned int version,
                 const unsigned int type,
...
error_code serverPacket::recv(tcp::socket &socket) noexcept
{
    error_code ec;
...
error_code serverPacket::send(tcp::socket &socket) noexcept
{
    lock_stream();
...
error_code serverPacket::process(tcp::socket &socket) noexcept
{
...
std::ostream &operator<<(std::ostream &os, const serverPacket &packet)
{
    os << "Packet: HDR {V:"
...
error_code serverPacket::welcome_client(tcp::socket &socket) noexcept
{
...
error_code serverPacket::process_client_command(const std::string cmd, tcp::socket &socket) noexcept
{
...
error_code serverPacket::acknowledge_client_response(tcp::socket &socket) noexcept
{
...
bool serverPacket::is_valid_version() const noexcept
{
    return (get_version() == MSG_VERSION);
}

bool serverPacket::is_valid_msg_type() const noexcept
{
